...
begin
  ...
  { intro h,
    ...
    { rintro x y hxy,
      ...
      exact ⟨a.1, (submonoid.mem_prod.1 ha).1, ((associated.prod _ _ _ _).1 ha₂).1, b.1, (submonoid.mem_prod.1 hb).1, ((associated.prod _ _ _ _).1 hb₂).1⟩ },
    { rintro x y hxy,
      specialize h (1,x) (1,y),
      rw prod.one_mk_mul_one_mk at h,
      rcases (h (submonoid.mem_prod.2 ⟨s.one_mem, hxy⟩)) with ⟨a, ha, ha₂, ⟨b, hb, hb₂⟩⟩,
      exact ⟨a.2, (submonoid.mem_prod.1 ha).2, ((associated.prod _ _ _ _).1 ha₂).2, b.2, (submonoid.mem_prod.1 hb).2, ((associated.prod _ _ _ _).1 hb₂).2⟩ } },

  { rintro ⟨hs, ht⟩ x y hxy,
    rcases ⟨(hs x.1 y.1 hxy.1), (ht x.2 y.2 hxy.2)⟩ with ⟨⟨z, hz, hz₂, ⟨z', hz', hz'₂⟩⟩, ⟨w, hw, hw₂, ⟨w', hw', hw'₂⟩⟩⟩,
    exact ⟨(z,w), submonoid.mem_prod.2 ⟨hz, hw⟩, (associated.prod _ _ _ _).2 ⟨hz₂, hw₂⟩, (z',w'), submonoid.mem_prod.2 ⟨hz', hw'⟩, (associated.prod _ _ _ _).2 ⟨hz'₂, hw'₂⟩⟩ },
end
